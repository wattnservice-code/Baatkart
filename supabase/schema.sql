...
-- ── Kommunikasjonslogg: hva som er sendt til hver bruker (CRM/bevis/dedup) ──────
-- Skrives server-side (n8n / Edge Function / e-post-webhook) med service_role.
-- Brukere kan lese egen historikk; admin ser alt via egen RPC.
create table if not exists public.communications (
  id          bigint generated by default as identity primary key,
  user_id     uuid not null references auth.users (id) on delete cascade,
  channel     text not null,        -- 'email' | 'sms' | 'push' | 'inapp'
  kind        text not null,        -- 'welcome' | 'report' | 'trial_ending' | 'payment_failed' | 'marketing' | ...
  subject     text,
  status      text not null default 'sent',  -- 'queued' | 'sent' | 'failed' | 'opened'
  provider_id text,                 -- ekstern melding-id (Resend/Stripe osv.)
  metadata    jsonb,                -- f.eks. {"period":"2026-06"}
  sent_at     timestamptz not null default now(),
  created_at  timestamptz not null default now()
);
create index if not exists communications_user_idx on public.communications (user_id, sent_at desc);
create index if not exists communications_kind_idx on public.communications (user_id, kind, sent_at desc);
alter table public.communications enable row level security;
grant select on public.communications to authenticated;
drop policy if exists "comm_select_own" on public.communications;
create policy "comm_select_own" on public.communications
  for select using (auth.uid() = user_id);
-- (ingen insert/update for brukere → kun service_role/backend skriver)
